  ...
  return function (target: object, propertyKey: string | symbol, descriptor: PropertyDescriptor): void {
    ...
    descriptor.value = async function (...args: unknown[]) {
      ...
      if (!breakerInstances.has(instanceKey)) {
        ...
        // Adicionando logs para cada evento do CircuitBreaker
        const eventsToLog = [
          'open', 'halfOpen', 'close', 'fire'
        ];

        eventsToLog.forEach(eventName => {
          breaker.on(eventName as any, (...eventArgs: unknown[]) => {
            console.log(`[CircuitBreaker] Evento: ${eventName} | Grupo: ${opt.group} | Classe: ${className}`);
          });
        });
      }
...
export function onEvent({ eventName, circuitGroup = 'default' }: OnEventInput) {
  return function (target: object, propertyKey: string | symbol): void {
    ...
    console.log('circuitEvents', circuitEvents);
  };
}
